...
#   the address of the caller instruction instead of the return instruction.
#   Also, before the subtraction the thumb bit is unset in ARM architectures.
# * Preserve lines of the traceback whose binary file was not found, but add
#   a warning suffix.
...
def SystemSymbolizerFactory(system, addr, binary, arch):
  if system == 'Darwin':
    ...
    return Addr2LineSymbolizer(binary, arch)
...
class SymbolizationLoop(object):
  ...
  def process_line_posix(self, line):
    ...
    if os.path.exists(binary):
      ...
      self.frame_no += 1
      return [self.current_line + " [Binary not found]"]
